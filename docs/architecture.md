...
Scroll Down Sports is a single-page application built with Vite and React. It uses React Router to move users from a date picker to a game list, and then into a game catch-up experience with play-by-play and social highlights.
...
| Game catchup | `src/pages/GameCatchup.tsx` | Full catch-up with PBP, social highlights, stats, and paced reveal.   |
...
Since accurate game start times aren't always available, posts are distributed proportionally:
...
Pacing is handled in `GameCatchup`:
